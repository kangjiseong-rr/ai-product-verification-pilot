...
| 검토 영역 | 번호 | 점검 항목 | 확인 내용 | 결과 | 확인 방법 |
| --- | --- | --- | --- | --- | --- |
| 인공지능 모델 확인 | 1.1 | 인공지능 적용 | 머신러닝, 딥러닝, LLM 등 인공지능 모델이 적용되어 있는가? | Y/N | 서류, 현장, 서류/현장 |
| 인공지능 모델 확인 | 1.2 | 학습/추론 특성 존재 | 모델에 학습 또는 추론 등 인공지능 특성이 존재하는가? | Y/N | 서류, 현장, 서류/현장 |
| 인공지능 모델 확인 | 1.3 | 추론 결과 확인 | 입력 데이터가 모델을 거쳐 유의미한 결과를 내는가? | Y/N | 서류, 현장, 서류/현장 |
| 인공지능 기능 확인 | 2.1 | 구조 | 아키텍처 설계도 상 인공지능 모델의 위치와 제품 내부 로직 결합 구조가 확인되는가? | Y/N | 서류, 현장, 서류/현장 |
| 인공지능 기능 확인 | 2.2 | 결과 반영 | AI 처리 결과가 최종 화면, 보고서, 추천, 분류 등 제품 결과에 반영되는가? | Y/N | 서류, 현장, 서류/현장 |
| 인공지능 기능 확인 | 2.3 | 추적 가능성 | 사용자 입력부터 AI 처리, 최종 출력까지 추적 가능한가? | Y/N | 서류, 현장, 서류/현장 |
| 외부 모델 연동 확인 | 3.1 | API 연동 | 외부 모델 연동 시 입/출력 예시 또는 연계 근거가 확인되는가? | Y/N | 서류, 현장, 서류/현장 |
